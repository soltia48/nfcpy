...
# The tag modules that use pycryptodome are imported lazily, from within
# nfc.tag.activate(). Import them up front, before 'platform' is replaced
# below, because loading the pycryptodome native library reads
# platform.architecture() and the replacement does not provide it.
...
if sphinx_rtd_theme:
    # The theme registers itself through an entry point, setting
    # html_theme_path with the deprecated get_html_theme_path() is not needed.
